...
export function handleStateEvent(e: any) {
	...
	let target = e.target.shadowRoot ? e.path[0] : e.target;
	// find the anchor
	const anchor = (target.closest || function (A: HTMLAnchorElement) {
	...
	if (
		...
		if (next.origin === curr.origin) {
			...
			const redirect = next.pathname + next.search;
			...
			if (redirect === (curr.pathname + curr.search)) {
				...
				const evt = <PushStateEvent>new CustomEvent('pushstate');
				evt.state = curr.href;
				...
				setTimeout(function () {
					...
					if ((<WindowWithPushState>window).onpushstate) {
						(<WindowWithPushState>window).onpushstate(evt);
					}
				});
...
export interface WindowWithPushState extends Window {
	onpushstate: (event: CustomEvent<any>) => void;
}

export interface PushStateEvent extends CustomEvent {
	state: string;
}
